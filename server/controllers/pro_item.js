...
const FastProds=require('../models/fastenerproducts');
...
exports.findByIdFastPro = (req, res) => {
	
	FastProds.findOne({ category: req.params.id })
	//.populate('metrics')
	.exec(function (err, proItem) {
		if (err){
			if(err.kind === 'ObjectId') {
			...
			return res.status(500).send({
				success:false,
				message: "Error retrieving Product item with given id" + err
			});
		}
